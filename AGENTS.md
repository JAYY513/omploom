...
omp-loom never imports `@oh-my-pi/*` or `@earendil-works/*` packages (they are
...
- **On-disk history** (`lib/subagent-history.ts`, `/api/sessions/[id]/subagents*`):
  ...
  task toolResults keep `progress[]`/`results[]` snapshots. omp-loom recovers
...
- The client sorts the merged list by most-recently-added (registration
  ...
  in `localStorage` (`omp-loom:expanded-projects`), defaulting to only the
...
- `GET /api/app-update` queries the npm registry for `omploom` updates, detects the install manager (`bun` vs `npm` via `detectInstallMethod`), and returns `updateAvailable` plus the exact terminal command (e.g. `npm install -g omploom` or `bun add -g omploom`).
...
- Auth flows go through RPC commands (`get_login_providers`, `login`) against the omp child process; credentials live in omp's `agent.db` (SQLite) which omp-loom never touches directly.
