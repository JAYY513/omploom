...
const { resolveAvailableThinkingLevels, selectableThinkingLevels, thinkingLevelIndex, thinkingLevelsForMeta } = await jiti.import("./thinking-levels.ts");
...
test("thinkingLevelIndex resolves slider positions with auto fallback", () => {
  assert.equal(thinkingLevelIndex(["auto", "off", "low"], "low"), 2);
  assert.equal(thinkingLevelIndex(["auto", "off", "low"], undefined), 0);
  assert.equal(thinkingLevelIndex(["auto", "off", "low"], "ultra"), 0);
});
