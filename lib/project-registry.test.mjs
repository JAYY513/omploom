...
async function withAgentDir(t) {
  const agentDir = mkdtempSync(join(tmpdir(), "omp-loom-registry-test-"));
...
test("validateProjectPath rejects empty, missing, and non-directory paths", (t) => {
  const root = mkdtempSync(join(tmpdir(), "omp-loom-path-test-"));
...
test("worktree paths canonicalize to the main repo root when registered", async (t) => {
  ...
  const root = mkdtempSync(join(tmpdir(), "omp-loom-registry-worktree-"));
  const repo = join(root, "repo");
  const worktreeBase = `${repo}-worktrees`;
  try {
    execFileSync("git", ["-c", "safe.directory=*", "init", repo], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "-c", "safe.directory=*", "config", "user.email", "omp-loom@example.invalid"], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "-c", "safe.directory=*", "config", "user.name", "omp-loom test"], { stdio: "ignore" });
...
test("resolveProject returns canonical on-disk casing for plain directories", (t) => {
  const root = mkdtempSync(join(tmpdir(), "omp-loom-canonical-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
