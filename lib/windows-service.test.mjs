...
test("getWebServiceLogPath returns expected log path", () => {
  ...
  assert.ok(logPath.endsWith(path.join("logs", "omp-loom-service.log")));
});

test("getWindowsShortcutPaths returns expected .lnk shortcut locations", () => {
  const shortcuts = getWindowsShortcutPaths();
  assert.ok(shortcuts.desktop.endsWith(path.join("Desktop", "omp-loom.lnk")));
  assert.ok(shortcuts.startMenu.endsWith(path.join("Programs", "omp-loom.lnk")));
  assert.ok(shortcuts.startup.endsWith(path.join("Startup", "omp-loom-tray.lnk")));
});
