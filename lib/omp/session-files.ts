...
// These paths are resolved and authorized at request time by omp-loom.
...
/**
 * Ceiling on the on-disk size omp-loom will materialize into memory. omp streams
...
// mutation omp-loom cares about. Stored on globalThis for hot-reload safety and
 ...
 * title_change audit entry — omp-loom only needs the display title, and a
...
export function writeSessionFileAtomicSync(filePath: string, body: string, tag = "rewrite"): void {
  ...
  const tempDir = mkdtempSync(path.join(dir, `.omp-loom-${tag}-`));
...
export function archiveSessionFileWithArtifacts(filePath: string, roots: SessionArchiveRoots = {}): string {
  ...
  const tempDir = mkdtempSync(path.join(path.dirname(destination), ".omp-loom-archive-"));
