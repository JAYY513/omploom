...
test("a models.yml price edit resolves fresh rates without a cache clear", async () => {
  const { resolveModelRates, clearRatesCache } = await loadSubject();
  clearRatesCache();

  const makeConfig = (input) => ({
    providers: {
      acme: { models: [{ id: "acme-large", cost: { input, output: 2, cacheRead: 0.5, cacheWrite: 1 } }] },
    },
  });

  const cfg = makeConfig(10);
  const first = resolveModelRates("acme", "acme-large", cfg);
  assert.equal(first.input, 10);
  // Same config object (one parse/sync run): the memo hits and the very same
  // rates object comes back — per-record resolution never recomputes.
  assert.equal(resolveModelRates("acme", "acme-large", cfg), first);

  // Same provider/model with an edited price — a fresh config object, exactly
  // what each readModelsConfig() call returns. The per-object memo must not
  // serve the rates the process saw first.
  const edited = resolveModelRates("acme", "acme-large", makeConfig(25));
  assert.equal(edited.input, 25);
});
