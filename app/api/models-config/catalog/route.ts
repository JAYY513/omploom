...
declare global {
  var __ompLoomModelsDevCatalogCache: CatalogCache | undefined;
}
...
async function loadCatalog(): Promise<ModelCatalogEntry[]> {
  let cache = globalThis.__ompLoomModelsDevCatalogCache;
  if (!cache) {
    cache = globalThis.__ompLoomModelsDevCatalogCache = { entries: [], expiresAt: 0 };
  }
