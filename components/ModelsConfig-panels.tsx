...
export function RetryFallbackDetail({ models }: { models: RuntimeModelEntry[] }) {
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 9 }}>
      ...
      <label style={{ padding: "10px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", color: "var(--text)", fontSize: 12, display: "flex", alignItems: "center", gap: 8 }}>{t("modelsConfig.retryAttempts")} <GlideSelect size="sm" value={String(retry.maxRetries ?? 10)} onChange={(v) => void save({ ...settings, retry: { ...retry, maxRetries: Number(v) } })} options={["0", "1", "2", "3", "5", "10", "15", "20"]} /></label>
      <label style={{ padding: "10px 12px", border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", color: "var(--text)", fontSize: 12, display: "flex", alignItems: "center", gap: 8 }}>{t("modelsConfig.returnToPrimary")} <GlideSelect size="sm" value={retry.fallbackRevertPolicy ?? "cooldown-expiry"} onChange={(v) => void save({ ...settings, retry: { ...retry, fallbackRevertPolicy: v as "cooldown-expiry" | "never" } })} options={[{ value: "cooldown-expiry", label: t("modelsConfig.afterCooldown") }, { value: "never", label: t("modelsConfig.never") }]} /></label>
    </div>
    <section style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", overflow: "hidden" }}>
      <div style={{ padding: "10px 12px", background: "var(--bg-panel)", display: "flex", alignItems: "center", gap: 8 }}><span style={{ color: "var(--text)", fontSize: 12, fontWeight: 600 }}>{t("modelsConfig.fallbackChainFor")}</span><GlideSelect size="sm" aria-label={t("modelsConfig.fallbackChainFor")} value={role} onChange={setRole} options={NATIVE_MODEL_ROLES} /></div>
      <div style={{ padding: 12, display: "flex", gap: 8 }}><GlideSelect aria-label={t("modelsConfig.selectFallbackModel")} value={candidate} onChange={setCandidate} options={[{ value: "", label: t("modelsConfig.selectFallbackModel") }, ...modelOptions.filter((value) => !chain.includes(value))]} style={{ flex: 1, minWidth: 0 }} /><button type="button" disabled={!candidate} onClick={() => { updateChain([...chain, candidate]); setCandidate(""); }} style={{ padding: "6px 10px", border: "none", borderRadius: "var(--radius-control)", background: "var(--accent-strong)", color: "white", cursor: "pointer", fontSize: 12 }}>{t("modelsConfig.add")}</button></div>
      {chain.length === 0 ? (
    ...
    {loading ? <div style={{ color: "var(--text-muted)", fontSize: 12 }}>{t("modelsConfig.loadingRoles")}</div> : NATIVE_MODEL_ROLES.map((role) => (
      <div key={role} className="model-role-row" style={{ display: "grid", gridTemplateColumns: "82px minmax(0, 1fr) minmax(110px, 0.35fr)", alignItems: "center", gap: 10, fontSize: 12 }}>
        ...
        {(() => {
          ...
          return <>
            <GlideSelect aria-label={`Model override for ${role}`} value={selectedModel} onChange={(v) => updateRoleModel(role, v)} options={[{ value: "", label: t("modelsConfig.noOverride") }, ...(!modelKnown && selectedModel ? [{ value: selectedModel, label: selectedModel, tag: "unavailable" }] : []), ...models.map((item) => ({ value: `${item.provider}/${item.id}`, label: `${item.name || item.id}`, tag: `${item.provider}/${item.id}` }))]} style={{ width: "100%", minWidth: 0 }} />
            <GlideSelect aria-label={`Thinking level for ${role}`} value={selectedThinking} onChange={(v) => updateRoleThinking(role, v)} disabled={!model} options={[{ value: "", label: t("modelsConfig.modelDefault") }, ...(model?.thinkingLevels ?? []).filter((level) => level !== "off")]} style={{ width: "100%", minWidth: 0 }} />
          </>;
        })()}
      </div>
    ))}
