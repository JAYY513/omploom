...
- Give the Usage model card **per-task model metrics** and extend its sort toggle to six keys. A "task" is one user prompt in one transcript (`(file_path, turn_index)`, branch siblings of the same prompt sharing a turn): each model row shows mean and lower-median **net-new** tokens per task (total minus cache reads — cache reads re-bill the running history and would measure conversation length, not task size), mean cost per task, and its task count. Sort by tokens, cost, avg or p50 tokens per task, cost per task, or task count; bars rescale to the active metric and the value the list is ordered by is highlighted in each row, so the active sort is legible from the rows alone. `USAGE_SCHEMA_VERSION` 3 re-parses synced rows for the new `turn_index` column.
- Add a full-page **Tasks** board (sidebar footer row + ⌘K/Ctrl+K command) spanning every workspace: a three-lane attention board — Running / Needs review / Recently read — with a summary bar of board-wide counts plus today's token and cost burn, project filter chips and a title search, collapsible lanes, and per-lane grouping by project (layout preferences persist per browser). Running cards carry a live stopwatch anchored to the session start and a context-usage gauge; runs started in external omp terminals are detected from transcript-write liveness (with a grace window bridging silent tool calls) even though they never enter the RPC registry.
- Count usage from **named omp profiles** (`omp --profile <name>`), which keep their own isolated session tree: totals, heatmap and streaks now cover every profile on the machine, and the dashboard shows a per-profile split when more than one has usage. The default profile is unchanged.
- Add a full-page **Usage** dashboard (sidebar row above Settings, ⌘K/Ctrl+K command) covering every omp run on the machine, not just the open session: total and peak tokens, longest chat duration, current and longest streaks, a 53-week token-activity heatmap (daily / weekly / cumulative), a 7-day / 30-day / all-time token trend, and model usage. A cold first open scans the session history in the background with a progress bar, and Refresh re-runs the sync. Subagent and advisor transcripts count toward every total (about 16% of a subagent-heavy history) and the split stays visible under the heatmap.
- Keep the Usage dashboard's range-scoped cards honest: the trend and model cards are labelled with the selected range and the model card states its own total, so a 7-day subset is never mistaken for the all-time figure in the hero row. The model card sorts by tokens or cost via a toggle (bars rescale to the active metric); ties fall back to the other metric. Each range is cached after its first visit (the others are prefetched in the background), and a first visit shows a loading state instead of stale numbers.
