# omploom — Product & Architecture Contract

## Purpose

omploom is a local, browser-based workspace for the
...
1. **OMP remains authoritative.** Sessions, credentials, providers, and agent
   behavior belong to the installed `omp` CLI. omploom must not invent a
...
3. **Node-first installation.** A normal user installs Node.js 22.19+ and OMP,
   then runs `npx omploom@latest` or installs `omploom` globally. omploom does not
...
- npm package and CLI command: `omploom`.
...
- `agent.db` contains authentication data. omploom never reads or writes it;
  authentication actions go through the OMP RPC process.
- A live OMP process owns writes to its session file. omploom routes supported
...
first. omploom does not provide a general editor for session JSONL or opaque OMP
state.
...
2. Confirm `npm pack --dry-run` contains the built `.next` output and exposes
   the `omploom` binary.
3. Publish `omploom@<version>` only from an npm account authorized for that
   package.
